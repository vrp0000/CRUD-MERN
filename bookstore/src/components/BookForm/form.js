...
const Form = (props) =>
{
    ...
    const onSubmit = (event, props) =>
    {
        ...
        fetch('/addbooks', {
            ...
            body: JSON.stringify({ title, price: _price, author, description, rating, category })
        }).then((response) =>
        {
        ...
        }).then(resp =>
        {
            if (resp.error === "Book already exists in the Database")
                ...
                fetch('/updatebook', {
                    ...
                    body: JSON.stringify({ title, price, author, description, rating, category })
                }).then((response) =>
                {
                    if (response.status === 200) {
    ...
    return (
        < form className="hdr" >
            ...
            <div className="sbmtbtns">
                ...
                <div>
                    <Link to={"/book-list/"}><button className="btn btn-primary">
                        Cancel
                    </button>
                    </Link>
                </div>
